...
개발자의 마인드 ? 기능을 만드는 사람이 아니라, 흐름을 설계하는 사람
이 요청은 어디까지 책임져야 하는가
비즈니스 로직은 어느 계층에 있어야 하는가
데이터는 어떤 흐름으로 이동해야 하는가
동시성 문제는 발생하지 않는가
장애가 생기면 어디서 복구할 것인가
확장되면 지금 구조가 유지 가능한가
...
Api 관리를 위한 다양한 기능들을 손쉽게 쓸 수 있게 제공해 주어서 편리함.
    ...
    🌟 <React> 🌟
      - useState 객체 또는 배열 수정 시 주의사항
          useState로 객체나 배열을 관리할 때는 직접 값을 수정하면 참조가 바뀌지 않아 React가 변경을 감지하지 못합니다. 
          그래서 항상 새로운 객체나 배열을 만들어서 setState에 전달해야 하고, 보통 스프레드 연산자나 map 같은 불변성 유지를 위한 메서드를 활용합니다.

      - 참조란? 
        참조는 객체나 배열 같은 데이터가 메모리에 저장된 위치(주소)를 의미합니다.
React는 state 변경 여부를 참조값으로 판단하기 때문에, 기존 객체/배열을 직접 수정하면 참조값이 안 바뀌어서 리렌더링이 안 될 수 있어요.
그래서 스프레드 연산자나 map을 사용해서 새로운 참조값을 만들어 줍니다

      - 단방향 데이터
        리액트가 단방향이라고 하는 건 언어가 단방향이라는 의미가 아니라, 데이터 흐름이 단방향(one-way data flow) 이라는 뜻입니다.
즉, 부모에서 자식으로만 props를 통해 데이터가 내려가고, 자식이 부모 데이터를 직접 수정할 수는 없습니다. 자식이 상위 상태를 바꾸고 싶으면 콜백 함수를 내려받아 호출해야 하죠.
이런 구조 덕분에 데이터 흐름을 추적하기 쉽고, 예측 가능한 상태 관리가 가능해집니다.
...
- Rest ,RestFull
 REST는 API를 설계하는 데 필요한 일종의 원칙이라고 생각합니다. 
 자원을 URI로 표현하고, HTTP 메서드를 행위에 맞게 사용하는 규칙이죠.
RESTful은 이 원칙을 잘 지켜서 만들어진 API를 의미합니다."
